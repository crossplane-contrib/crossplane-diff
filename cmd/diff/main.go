...
type CommonCmdFields struct {
	...
	MaxRecvMessageSize       int                 `default:"0"                                                                                       help:"Max gRPC message size (MB) for render function containers. 0 leaves the function default (4MB). Falls back to the CROSSPLANE_DIFF_MAX_RECV_MESSAGE_SIZE env var when unset." name:"max-recv-message-size"`
