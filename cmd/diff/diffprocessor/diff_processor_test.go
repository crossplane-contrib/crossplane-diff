...
// TestDefaultDiffProcessor_EventualState_Integration verifies that the EventualState option
// is properly integrated into DiffSingleResource. This tests that:
// 1. The EventualState flag controls whether simulation runs
// 2. The simulation augments observed resources with Ready status
// 3. The augmented resources are passed to the final render.
func TestDefaultDiffProcessor_EventualState_Integration(t *testing.T) {
	ctx := t.Context()
	...
	tests := map[string]struct {
		eventualState     bool
		setupRenderFunc   func() (RenderFunc, *int, *[]int)
		wantRenderCalls   int
		wantFinalObserved int
	}{
		"EventualStateDisabled_SingleRenderCall": {
			eventualState: false,
			setupRenderFunc: func() (RenderFunc, *int, *[]int) {
				renderCount := 0
				observedCounts := []int{}

				fn := func(_ context.Context, _ logging.Logger, in render.Inputs) (render.Outputs, error) {
					renderCount++

					observedCounts = append(observedCounts, len(in.ObservedResources))
				...
				return fn, &renderCount, &observedCounts
			},
			wantRenderCalls:   1,
			wantFinalObserved: 0,
		},
		"EventualStateEnabled_SimulationRuns": {
			eventualState: true,
			setupRenderFunc: func() (RenderFunc, *int, *[]int) {
				renderCount := 0
				observedCounts := []int{}

				// Simulate function-sequencer behavior:
				// - First render (simulation): return stage-1 resource
				// - Second render (simulation): return stage-1 + stage-2 (stage-1 now "ready")
				// - Third render (simulation): same as second (stable)
				// - Fourth render (actual): final render with augmented observed
				fn := func(_ context.Context, _ logging.Logger, in render.Inputs) (render.Outputs, error) {
					renderCount++
					...
					// Use s3.aws.crossplane.io/v1 Bucket which won't trigger nested XR processing
					switch observedCount {
					case 0:
						// First render: return stage 1 resource
						stage1 := cpd.New()
						stage1.SetName("stage-1-bucket")
						stage1.SetAPIVersion("s3.aws.crossplane.io/v1")
						stage1.SetKind("Bucket")
						stage1.SetAnnotations(map[string]string{
							"crossplane.io/composition-resource-name": "stage-1",
						})

						return render.Outputs{
							CompositeResource: in.CompositeResource,
							ComposedResources: []cpd.Unstructured{*stage1},
						}, nil
					case 1:
						...
						stage2 := cpd.New()
						stage2.SetName("stage-2-bucket")
						stage2.SetAPIVersion("s3.aws.crossplane.io/v1")
						stage2.SetKind("Bucket")
						stage2.SetAnnotations(map[string]string{
							"crossplane.io/composition-resource-name": "stage-2",
						})

						return render.Outputs{
							CompositeResource: in.CompositeResource,
							ComposedResources: []cpd.Unstructured{*stage1, *stage2},
						}, nil
					default:
						// Third+ render: stable state
						stage1 := cpd.New()
						stage1.SetName("stage-1-bucket")
						stage1.SetAPIVersion("s3.aws.crossplane.io/v1")
						stage1.SetKind("Bucket")
			...
			// 3 simulation renders + 1 final render = 4 total
			wantRenderCalls: 4,
			// Final render should receive 2 augmented observed resources
			wantFinalObserved: 2,
		},
	}

	// Create Bucket CRD for composed resources
	bucketCRD := tu.NewCRD("buckets.s3.aws.crossplane.io", "s3.aws.crossplane.io", "Bucket").
		WithListKind("BucketList").
		WithPlural("buckets").
		WithSingular("bucket").
	...
	for name, tt := range tests {
		t.Run(name, func(t *testing.T) {
			renderFn, renderCount, observedCounts := tt.setupRenderFunc()

			// Create empty resource tree (new XR)
			emptyTree := &resource.Resource{
				Unstructured: *xr,
				Children:     []*resource.Resource{},
			}
			...
			k8sClients := k8.Clients{
				Apply: tu.NewMockApplyClient().
					...
					Build(),
				Resource: tu.NewMockResourceClient().
					WithResourceNotFound(). // New XR doesn't exist
					Build(),
				Schema: tu.NewMockSchemaClient().
					WithNoResourcesRequiringCRDs().
					WithGetCRD(func(_ context.Context, gvk schema.GroupVersionKind) (*extv1.CustomResourceDefinition, error) {
						switch {
						case gvk.Group == testGroup && gvk.Kind == "XR":
							return xrCRD, nil
						case gvk.Group == "s3.aws.crossplane.io" && gvk.Kind == "Bucket":
							return bucketCRD, nil
						default:
			...
			xpClients := xp.Clients{
				...
				Definition: tu.NewMockDefinitionClient().
					WithGetXRDForXR(func(_ context.Context, gvk schema.GroupVersionKind) (*un.Unstructured, error) {
						// Only return XRD for the XR type, not for composed resources like Bucket
						if gvk.Group == testGroup && gvk.Kind == "XR" {
							return xrdUnstructured, nil
						}
						// Return not found for all other types (Bucket, etc.)
			...
			// Create processor with EventualState option
			baseOpts := testProcessorOptions(t)
			customOpts := []ProcessorOption{
				WithEventualState(tt.eventualState),
			...
			_, err = processor.(*DefaultDiffProcessor).DiffSingleResource(ctx, xr, compositionProvider)
			if err != nil {
				t.Fatalf("DiffSingleResource() error = %v", err)
			}

			// Verify render call count
			if *renderCount != tt.wantRenderCalls {
				t.Errorf("Expected %d render calls, got %d", tt.wantRenderCalls, *renderCount)
			}

			// Verify final observed count (last element in observedCounts)
			if len(*observedCounts) > 0 {
				finalObserved := (*observedCounts)[len(*observedCounts)-1]
				if finalObserved != tt.wantFinalObserved {
					t.Errorf("Final render received %d observed resources, want %d (all observed counts: %v)",
						finalObserved, tt.wantFinalObserved, *observedCounts)
				}
			}

			// For EventualState enabled, verify the observed resources have Ready status
			if tt.eventualState && len(*observedCounts) > 1 {
				// The second render should have received observed resources with Ready condition
				// (from simulation augmentation)
				if (*observedCounts)[1] != 1 {
					t.Errorf("Second simulation render should have received 1 observed resource with Ready status, got %d",
						(*observedCounts)[1])
				}
			}
		})
