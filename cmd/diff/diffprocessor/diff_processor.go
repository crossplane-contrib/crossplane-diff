...
func (p *DefaultDiffProcessor) resolveBackingXRForClaim(ctx context.Context, existingXRFromCluster *un.Unstructured, xr *cmp.Unstructured) (backingXRInfo, error) {
	...
	if !hasResourceRef || resourceRefRaw == nil {
		p.config.Logger.Debug("No spec.resourceRef in cluster resource (either not a Claim, or backing XR not yet bound)",
			"resource", xr.GetName(),
			"gvk", existingXRFromCluster.GroupVersionKind().String())
		return result, nil
	}
